...
import server from '../src/server/Interview-server';
...
describe('Chat Interview application', () => {
    ...
    const aBadRequest = () => request(baseUrl).get(Urls.DEFAULT);

    const aGoodRequest = url => request(baseUrl)
        .get(url)
        .set(anAuthConfiguration);

    it('should throw an exception is missing mandatory authentication header', async () => {
        const result = await aBadRequest();
    ...
    it('should be able to clear all messages', (done) => {
        aGoodRequest(Urls.MESSAGES)
            ...
            }).then(() => request(baseUrl).post(Urls.CLEAR).set(anAuthConfiguration)).then(() => {
